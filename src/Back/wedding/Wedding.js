...
const Kategori = () => {
    ...
    function setIsiTabel() {
        let nomor = 1
        const datawedding = isi.map((val, index) => ({
            no: nomor++,
            jenis_wedding: val.jenis_wedding,
    ...
    async function hapus(id) {
        if (window.confirm('yakin akan menghapus?')) {
            const res = await link.delete('/wedding/' + id);
            setPesan(res.data.pesan);
        }
    }


    async function showData(id_wedding) {
        history.push({

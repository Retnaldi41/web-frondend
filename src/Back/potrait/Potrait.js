...
const Kategori = () => {
    ...
    const [id, setIdpotrait] = useState('');
    ...
    function setIsiTabel() {
        let nomor = 1
        const dataPotrait = isi.map((val, index) => ({
            no: nomor++,
            jenis_potrait: val.jenis_potrait,
    ...
    async function hapus(id) {
        if (window.confirm('yakin akan menghapus?')) {
            const res = await link.delete('/potrait/' + id);
            setPesan(res.data.pesan);
        }
    }

   
    async function showData(id_potrait) {       
        history.push({
            pathname: '/admin/showPotrait',
            state: { id: id_potrait }                       
        });            
    }

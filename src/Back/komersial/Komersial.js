...
const Kategori = () => {
    ...
    const [id, setIdkomersial] = useState('');
    ...
    const columns = [
        ...
        {
            name: "Jenis Komersial",
            selector: "jenis_komersial",
            sortable: true
        },
        {
            name: "Fasilitas",
    ...
    function setIsiTabel() {
        let nomor = 1
        const dataKomersial = isi.map((val, index) => ({
            no: nomor++,
            jenis_komersial: val.jenis_komersial,
            fasilitas: val.fasilitas,
            harga: val.harga,
            aksi: val.id_komersial,
        }))
        setData(dataKomersial)
    }       

    async function hapus(id) {
        if (window.confirm('yakin akan menghapus?')) {
            const res = await link.delete('/komersial/' + id);
            setPesan(res.data.pesan);
        }
    }

   
    async function showData(id_komersial) {       
        history.push({
            pathname: '/admin/showKomersial',
            state: { id: id_komersial }                       
        });            
    }
        ...
        <main id="main" class="main">
            <div>
                ...
                <div className="row">
                    <p>{pesan}</p>                    
                    <Link to={{
                        pathname: "/admin/createKomersial"                       

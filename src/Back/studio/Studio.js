...
const Kategori = () => {
    ...
    function setIsiTabel() {
        let nomor = 1
        const dataStudio = isi.map((val, index) => ({
            no: nomor++,
            jenis_studio: val.jenis_studio,
    ...
    async function hapus(id) {
        if (window.confirm('yakin akan menghapus?')) {
            const res = await link.delete('/studio/' + id);
            setPesan(res.data.pesan);
        }
    }

   
    async function showData(id_studio) {       
        history.push({
            pathname: '/admin/showStudio',
            state: { id: id_studio }                       
        });            
    }

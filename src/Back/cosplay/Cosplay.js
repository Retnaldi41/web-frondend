...
const Kategori = () => {
    ...
    function setIsiTabel() {
        let nomor = 1
        const dataCosplay = isi.map((val, index) => ({
            no: nomor++,
            jenis_cosplay: val.jenis_cosplay,
    ...
    async function hapus(id) {
        if (window.confirm('yakin akan menghapus?')) {
            const res = await link.delete('/cosplay/' + id);
